        ...
        contactForm.addEventListener('submit', async function(e) {
            ...
            Object.keys(validationRules).forEach(fieldName => {
                ...
                try {
                    ...
                    const isBackendAvailable = await window.kristechAPI.isBackendAvailable();
                    
                    if (isBackendAvailable) {
                        // Submit to API if available
                        const response = await window.kristechAPI.submitContactForm(formData);
                        
                        // Success
                        showNotification(response.message || 'Thank you for your message! We\'ll get back to you within 24 hours.', 'success');
                        contactForm.reset();
                    } else {
                        // Fallback to mailto: if backend is not available
                        const email = 'christophermen60@gmail.com';
                        const subject = encodeURIComponent(formData._subject);
                        const body = encodeURIComponent(
                            ...
                            `Message:\n${formData.message}\n\n` +
                            `Privacy Policy Accepted: ${formData.privacy === 'true' ? 'Yes' : 'No'}`
                        );
                        ...
                        showNotification('Your default email client has been opened. Please send the pre-filled email to contact us.', 'info');
                    }
                    ...
                    if (typeof gtag !== 'undefined') {
                        gtag('event', 'form_submit', {
                            ...
                            value: isBackendAvailable ? 'api' : 'email_fallback'
                        });
                    }
                    ...
                    if (error.isNetworkError) {
                        // Fallback to email client for network errors
                        const email = 'christophermen60@gmail.com';
                        const subject = encodeURIComponent('Contact Form Submission - Fallback');
                        const body = encodeURIComponent(
                            'I tried to submit the contact form but there was an error. Here\'s my message:\n\n' +
                        ...
                        errorMessage += 'We had trouble connecting to our server. ';
                        errorMessage += 'Your default email client has been opened with a pre-filled message. ';
                        errorMessage += 'Please send the email to contact us directly.';
